...
class SPLoss(nn.Module):
    def __init__(self, n_samples):
        ...
        self.threshold = 2 # adjust by mean emprical loss
        self.growing_factor = 1.25
...
def init_logger(log_dir: str, log_file: str) -> None:
    ...
    logger.handlers = []  # 清除之前的所有 handlers
...
#argsort() 函数返回的是数组中元素的 索引 排序。对于 -pred_list，它会返回一个按负得分从小到大的索引顺序
...
class EarlyStopping:
    ...
    def update(self, epoch: int, test_hr_a_10: float, test_hr_b_10: float,
               test_ndcg_a_10: float, test_ndcg_b_10: float) -> int:
        # 仅当有任何一个指标提升时，才更新
        improved = False

        # Check and update HR@10 metrics
        if test_hr_a_10 > self.best_hr_a_10:
            self.best_hr_a_10 = test_hr_a_10
            improved = True

        if test_hr_b_10 > self.best_hr_b_10:
            self.best_hr_b_10 = test_hr_b_10
            improved = True

        # Check and update NDCG@10 metrics
        if test_ndcg_a_10 > self.best_ndcg_a_10:
            self.best_ndcg_a_10 = test_ndcg_a_10
            improved = True

        if test_ndcg_b_10 > self.best_ndcg_b_10:
            self.best_ndcg_b_10 = test_ndcg_b_10
            improved = True
